...
x_pid = PID.PID(P=0.145, I=0.0, D=0.0007)  # pid initialization
...
def getAreaMaxContour(contours):
    ...
    for c in contours:  # iterate through all contours
        ...
        if contour_area_temp > contour_area_max:
            ...
            # only contours with an area greater than 100 are considered valid; the contour with the largest area is used to filter out interference
...
# 启动作线程(start the thread for executing actions)
...
def run(img):
    ...
    if areaMaxContour is not None and area_max > 100:
        ...
        # get diagonal points of rectangle
...
if __name__ == '__main__':
    ...
    start()
    __target_color = ('red')
    # Try to read camera config, fall back to default
    try:
