...
- Periodically pauses to scan for a red intruder
- If red is detected, switches into Follow mode in-process
- After a short follow window, returns to patrol mode

Place this file in the same folder as Follow.py on the robot.
"""
...
def start_follow_mode(start_pitch, start_yaw):
    ...
    try:
        # Refresh Follow's config so its LAB thresholds are available.
        Follow.load_config()

        # Force Follow's runtime defaults to match the pose where red was seen.
        Follow.servo_data['servo1'] = int(start_pitch)
        Follow.servo_data['servo2'] = int(start_yaw)

        # Keep Follow's internal tracking position aligned too.
    ...
    try:
        # Hold the head where the intruder was detected.
        Board.setPWMServoPulse(1, int(start_pitch), 150)
        Board.setPWMServoPulse(2, int(start_yaw), 150)
    except Exception:
        pass

    # Start Follow mode in-process.
    Follow.start()
    # IMPORTANT: set target color after Follow.start(), because start() resets it.
...
def scan_for_intruder(my_camera):
    ...
    # Wide, slower, more dramatic scan
    for pos in range(servo_data['servo2'] - 450, servo_data['servo2'] + 451, 80):
        ...
        # Keep preview updating during scan so the feed does not appear frozen.
